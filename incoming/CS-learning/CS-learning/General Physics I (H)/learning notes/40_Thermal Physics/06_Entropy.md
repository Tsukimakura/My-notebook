...
- **Clausius Inequality:** For any cyclic process, the integral of heat transfer divided by temperature is less than or equal to zero:
    ...
    - **Reversible process:** $\oint \frac{đQ}{T} = 0$ (Clausius equality)
        
    - **Irreversible process:** $\oint \frac{đQ}{T} < 0$
...
- **Proof via Heat Engine:** Consider an engine working between two constant-$T$ reservoirs.
    ...
    - Generalizing to an arbitrary quasi-static cyclic process with multiple reservoirs: $\sum_i \left( \frac{\Delta Q_H^{(i)}}{T_H^{(i)}} + \frac{\Delta Q_L^{(i)}}{T_L^{(i)}} \right) \leq 0 \implies \oint \frac{đQ}{T} \leq 0$.
        
- **Definition of Entropy ($S$):** Because $\oint \frac{đQ}{T} = 0$ for a reversible cycle, the integral between two states is independent of the path taken. This allows us to define a state function called entropy.
    
    - **Integral form:** $S_B - S_A = \int_A^B \frac{đQ}{T}$
        
    - **Differential form:** $dS = \frac{đQ}{T}$
        
    - _Note on differentials:_ Heat ($đQ$) is an inexact differential (path-dependent), whereas entropy ($dS = \frac{đQ}{T}$) is an exact differential (path-independent).
...
$$\int_{A(I)}^B \frac{đQ}{T} + \int_{B(II)}^A \frac{đQ}{T} < 0$$

Since the return path is reversible, $\int_{B(II)}^A \frac{đQ}{T} = S_A - S_B$. Substituting this yields the general relation for any path from A to B:

$$S_B - S_A \geq \int_A^B \frac{đQ}{T}$$

- **Application to Isolated Systems:** An isolated system exchanges no matter or energy (heat) with its surroundings, meaning $đQ = 0$ (adiabatic). Therefore:
...
- **1. Ideal Gas:**
    ...
    - First Law: $dU = đQ - P dV \implies T dS - P dV$
...
- **3. Heating Water:**
    ...
    - Imagine a quasi-static heating process: $đQ = m c_v dT$
...
- From $dS = \frac{đQ}{T}$, we get $đQ = T dS$.
