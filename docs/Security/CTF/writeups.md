...
3. 搜索下载的git仓库
	...
	- ......
 1. 根据文件找到flag，题中为一串MD5加密([[MD5 | 参见MD5笔记]])的内容，通过cmd5.com在线解密
 2. **继续学习**：运维的基本素养--怎么配置Apache或者Nginx防止出现git leak
...
（[[端口（Port）| 什么是端口]])
1. 题目提示： 以下所有域名均应在 SOCSK5 代理后才可访问(可以使用 proxychains4 等工具)
    ...
    ```
2. 使用nmap 按照题目要求扫描zju.tools(192.168.192.3)在9000-11000范围内的SSH端口：[[proxychains4#基本使用方法 | proxychains4使用方法]] [[端口扫描与nmap]]
```bash
# 使用proxychains通过代理扫描
...
1. 根据提议访问` http://192.168.192.8:[part1扫描出来的端口号]`
```bash
