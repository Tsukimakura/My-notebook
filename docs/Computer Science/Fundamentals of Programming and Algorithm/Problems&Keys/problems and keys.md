...
3. **关于C语言指针的运算：指针只有加减操作，没有乘除操作。指针可以加常数、减常数；相同类型的指针可以相加、相减。(F)**
...
4. **The remainder operator (%) can be used only with integer operands. (T)**

5. **The following code fragment prints out .**
